...
const SuratUndangan = () => {
  ...
  // Fetch templates kustom untuk surat undangan
  useEffect(() => {
    const fetchTemplates = async () => {
      setLoadingTemplates(true);
      try {
        const response = await api.get('/dashboard/templates/by-type/surat_undangan');
          ...
          <div className="p-8 md:p-12">
            ...
            <div className="md:col-span-2 md:grid-cols-2 gap-8 mb-8">
              ...
              <input
                ...
                placeholder="Contoh: 005/UND/X/2025"
                className="w-full border border-[#E5DED5] rounded-xl p-3.5 focus:ring-4 focus:ring-[#B28D35]/10 focus:border-[#B28D35] outline-none"
              />
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
              <div>
                ...
                <select name="jenis_surat" value={formData.jenis_surat} onChange={handleChange} className="w-full border border-[#E5DED5] rounded-xl p-3.5 outline-none bg-[#FDFBF7]/50 cursor-pointer" disabled={loadingTemplates}>
            ...
            <div className="flex flex-col md:flex-row gap-3 mb-6 items-end">
              ...
              <button onClick={addRecipient} className="bg-[#B28D35] text-white px-8 py-3.5 rounded-xl font-bold hover:bg-[#96762B]">
                Tambah
              </button>
            </div>
              ...
              <div className="grid grid-cols-1 gap-3">
                {recipients.map((item, idx) => (
                  <div key={idx} className="flex justify-between items-center bg-white p-4 rounded-xl border border-[#E5DED5] shadow-sm">
                    ...
                    <button onClick={() => removeRecipient(idx)} className="text-rose-500 hover:bg-rose-50 px-3 py-1.5 rounded-lg text-sm font-bold">
                      Hapus
                    </button>
                  </div>
                ))}
